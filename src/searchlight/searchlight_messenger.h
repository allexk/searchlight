 ...
 * Then, each user should call init() to initialize it (only the first call
 * will come through). The Messenger will deactivate itself automatically
 * via the query finalizer.
 *
 ...
 * carriers only.
 *
 * TODO: For now the messenger does not work if multiple queries use
 *     searchlight operator. To be more precise, there is no isolation between
 *     different queries. If a query ends, it deactivates the messenger via
 *     the registered finalizer. This can be remedied by allowing mapping
 *     between query IDs and context information (e.g., registered arrays).
 *     Query ID is always present in the message.
 */
...
public:
     ...
     */
    void init(const boost::shared_ptr<Query> &query);
     ...
     */
    void RegisterArray(const ArrayPtr &array);
     ...
     */
    bool RequestChunk(InstanceID inst, const std::string &array_name,
...
private:
    ...
    void deactivate(const boost::shared_ptr<Query> &);
    ...
    // Has the messenger been initialized?
    bool initialized_ = false;

    // The SciDb query we are working with
    boost::weak_ptr<Query> query_;

    // Arrays registered for chunk exchange
    std::unordered_map<std::string, ArrayPtr> reg_arrays_;
