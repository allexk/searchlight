...
void SolvePointProblem(const SimpleArray2D &array) {
    Solver solver("array point problem");
    ...
    // looking for a point
    lens[0] = solver.MakeIntVar(1, 1, "lx");
    lens[1] = solver.MakeIntVar(1, 1, "ly");
    ...
    // add the main constraint: sum <=0 or sum >= 9
    IntVar * const b1 = solver.MakeIsLessOrEqualCstVar(sum_var, 0);
    IntVar * const b2 = solver.MakeIsGreaterOrEqualCstVar(sum_var, 9);
    solver.AddConstraint(solver.MakeEquality(solver.MakeSum(b1, b2), 1));
